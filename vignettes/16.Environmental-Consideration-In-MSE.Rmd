...
# 2. Generate basic information for the MSE
...
# 3. Configure population random effects (NAA_re)
...
# 4. Environmental covariate (Ecov): data, process model, and recruitment effect
...
# 5. Generate WHAM input and build the OM (no fitting)
...
# 6. Where Ecov objects live in the simulated OM (direct access)

After simulation with `update_om_fn()`:

- Ecov observation matrix: `om_seed$input$data$Ecov_obs`
- “True” Ecov state (latent state): `om_seed$rep$Ecov_x`
- Latent deviations: `om_seed$parList$Ecov_re` (if simulated/used)
- Ecov year vector (often): `om_seed$years_Ecov` (or `om_seed$input$years_Ecov`)

If any of these are `NULL` in your local setup, run `str(om_seed, 2)` and adjust
the paths accordingly.

# 7. Example A: TWO different true states across realizations
...
# 8. Example B: ONE fixed true state, but Ecov observations differ across realizations
...
- the true state (`Ecov_x`) is identical across realizations (by construction),
- observations (`Ecov_obs`) can still differ across realizations due to
  observation error.

## 8.1 Fix `Ecov_re`, build OM, simulate two realizations
...
# 9. Run the MSE feedback loop (using Example B)
