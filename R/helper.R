...
#' and realization, relative to the corresponding operating model values for
#' that year. SSB is row-summed across regions for both OM and EM; F uses the
#' global F column (last column of \code{Fbar}) in both OM and EM.
#'
...
#' @param width,height Figure size passed to \code{ggsave()}.
...
#'   \item The EM terminal year index is interpreted as the year index in the
#'         OM as well; the function aligns OM and EM by this index.
#' }
#'
#' @keywords internal
plot_terminal_ssb_f_bias_by_assessment <- function(
  ...
  # Helper for ordinal label if you want "1st, 2nd, 3rd..."
  ordinal <- function(k) {
  ...
  for (k in seq_len(n_assess)) {
    ...
    if (!is.nsim) {
      ...
      for (m in seq_along(mods)) {
        ...
        if (is.null(em_list) || length(em_list) < k || is.null(em_list[[k]]))
          next
