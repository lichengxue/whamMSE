...
#' @examples
#' \dontrun{
#' em_info <- some_estimation_model_info
#' em_years <- 1990:2020
#' basic_info <- generate_basic_info_em(em_info, em_years)
#' }
#'
generate_basic_info_em <- function(em_info, 
  ...
  if (is.null(filter_indices) || all(filter_indices != 0)) {
    ...
    if (any(filter_indices == 0)) {
      ...
      # 
      # if(length(em_info$par_inputs$index_cv) == n_indices_om) em_info$par_inputs$index_cv = em_info$par_inputs$index_cv[idx]
      # if(length(em_info$par_inputs$index_Neff) == n_indices_om) em_info$par_inputs$index_Neff = em_info$par_inputs$index_Neff[idx]
      # if(length(em_info$par_inputs$fracyr_indices) == n_indices_om) em_info$par_inputs$fracyr_indices = em_info$par_inputs$fracyr_indices[idx]
      # if(length(em_info$par_inputs$q) == n_indices_om) em_info$par_inputs$q = em_info$par_inputs$q[idx]
      # if(length(em_info$par_inputs$use_indices) == n_indices_om) em_info$par_inputs$use_indices = em_info$par_inputs$use_indices[idx]
      # if(length(em_info$par_inputs$use_index_paa) == n_indices_om) em_info$par_inputs$use_index_paa = em_info$par_inputs$use_index_paa[idx]
      # if(length(em_info$par_inputs$units_indices) == n_indices_om) em_info$par_inputs$units_indices = em_info$par_inputs$units_indices[idx]
      # if(length(em_info$par_inputs$units_index_paa) == n_indices_om) em_info$par_inputs$units_index_paa = em_info$par_inputs$units_index_paa[idx]
      # 
      # if(!is.null(em_info$par_inputs$index_regions)) {
      #   index_regions = em_info$par_inputs$index_regions
      #   index_regions = index_regions[idx]
      #   unique_values = unique(index_regions)
      #   new_values = seq_along(sort(unique(index_regions)))
      #   mapping <- setNames(new_values,unique_values)
      #   index_regions <- as.integer(mapping[as.character(index_regions)])
      #   em_info$par_inputs$index_regions <- index_regions
      # }
